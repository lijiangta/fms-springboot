...
public interface SeuMapper {
    @InsertProvider(type = SeuSqlProvider.class, method = "addSeuSql")
    Integer addSeu(AddSeuDto addSeuDto);

    @SelectProvider(type = SeuSqlProvider.class, method = "getSeu")
    SeuDto getSeu(SelectSeoVo selectSeo);

    @UpdateProvider(type = SeuSqlProvider.class, method = "setSeu")
    Integer setSeu(UpdateSeoVo updateSeo);

    @DeleteProvider(type = SeuSqlProvider.class, method = "delSeu")
    Integer delSeu(DeleteSeoVo deleteSeo);
